#!/usr/bin/env python3
"""Start the WebUI-local relay and multiple synthetic video publishers.

This script intentionally owns relay/publisher lifecycle outside the WebUI
server. The browser UI only subscribes/unsubscribes tracks and renders video.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Start WebUI relay and synthetic video publishers")
...
async def main_async() -> None:
    ...
    try:
        ...
        publishers = [await start_publisher(source, args.host, args.port) for source in sources]
        print("press Ctrl+C to stop relay and publishers", flush=True)
        await stop_event.wait()
    finally:
        for process in publishers:
            await terminate_process(process)
        await relay.stop()
        print("relay and publishers stopped", flush=True)
